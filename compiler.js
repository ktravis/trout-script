...
Compiler.prototype.resolve_node = function (node) {
  ...
  switch(node._type) {
    ...
    case "inferred anonymous block function": //output += ; break;
    case "anonymous block function": //output += ; break;
    case "inferred function declaration": //output += ; break;
    case "function declaration": output = this.resolve_function_declaration(node); break;

    case "capture block": //output += ; break;
    case "non capture block": //output += ; break;
    ...
    // case "while statement": output += ; break;
...
Compiler.prototype.resolve_function_declaration = function (node) {
	...
	if (node._type.indexOf("anonymous") === -1) {
	  ...
	  str = this.scope_str+".declare('"+node.name+"','"+type_str+"',false);\n";
	  for (var i = 0; i < this.indent; i++)
	  	str += '  ';
	  str += this.scope_str+'.assign_variable("'+node.name+'",runtime.create_function('+argnames+', '+argtypes+', "'+return_type+'", '+deps+', function (scope) {\n'+block.str+'}))';
	} else
...
Compiler.prototype.resolve_variable = function(node) {
  ...
  if (this.has_local(node.name))
    ...
    return Node(this.scope_str+".get_variable_absolute('"+node.name+"')", v.type, node, [node.name]);
  }
};
Compiler.prototype.resolve_reference = function(node) {
  ...
  return Node('new TrObject("&'+of.type+'", '+this.scope_str+'.get_reference("'+node.of.name+'"))', '&'+of.type, node, node.of.dependencies);
};
Compiler.prototype.resolve_expression = function(node) {
};
Compiler.prototype.resolve_call = function(node) {
  ...
  for (var i = 0; i < args.length; i++) {
    if (!check_types(callee_fn_type.args[i],args[i].type))
      ...
      arg_str.push(args[i].str+".copy()"); // should this be handled in the runtime?
    else
...
Compiler.prototype.resolve_assignment = function(node) {
  ...
  if (node.lhs._type === 'member') {
    ...
    str += this.scope_str+".set_member("+of.str+",\'"+node.lhs.property+"\',"+rhs.str+")";
  } else {
    if (this.get_var(node.lhs.name).const)
      this.fail("Reassignment of constant variable '"+node.lhs.name+"'.");
    str += this.scope_str+".assign_variable('"+lhs.orig.name+"',"+rhs.str+")";
  }
...
Compiler.prototype.resolve_declaration = function(node) {
  ...
  // TODO: some kind of check that type exists here?
  ...
  if (node._type === 'declaration')
    ...
    return Node(this.scope_str+".declare_const('"+node.name+"',"+val.str+")", type, node, deps);
  else {
    var str = this.scope_str+".declare('"+node.name+"','"+type+"',false);\n";
    for (var i = 0; i < this.indent; i++)
      str += '  ';
    str += this.scope_str+".assign_variable('"+node.name+"',"+val.str+")";
